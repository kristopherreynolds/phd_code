...
for ii = 1 : size(pt_set,1)
    ...
    m = pt_set(ii,1);
    n = pt_set(ii,2);
    thi_current = thi_anl2(m,n,0,a,b,c,2);
...
for ipass = 1 : M
    ...
    for ipts = 1 : size(ref_pts,1)
        ...
        thi_p1 = thi_2d_flop(m,n,a,b,c,thi_m1);
        %compare result to true value
        disp(['(m,n) = (',num2str(m),',',num2str(n),')'])
        thi_p1true = [thi_anl(m+1,n,0,a,b,c,2);thi_anl(m,n+1,0,a,b,c,2)];
        disp('RR')
        disp(thi_p1)
        disp('True Value')
        disp(thi_p1true)
        ...
        %store error
        errvec = [errvec;(norm(thi_p1-thi_p1true))];
