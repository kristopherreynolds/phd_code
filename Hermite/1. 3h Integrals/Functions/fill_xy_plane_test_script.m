...
[ax_x,ax_y,~] = fill_thi_axes(N,a,b,c);
...
%perform passes
for ipass = 1 : 2 : M %skip every other pass 
    ...
    newref_pts = [(ipass+1:-1:1)',(1:ipass+1)'];
...
%         hold on
%         %plot(xv,yv,'k*')
%         plot(pt_set(:,1),pt_set(:,2),'k*',ref_point_current(1),ref_point_current(2),'mo',...
%             new_pts(:,1),new_pts(:,2),'go',old_pts(:,1),old_pts(:,2),'ro')
...
%         plot([N N],[0 N],'k')
%         plot([0 N],[N N],'k')
%         set(gca,'xlim',[-0.5 N+2],'ylim',[-0.5 N+2])
%         grid on
%         xlabel('m')
%         ylabel('n')
%         caxis([-0.3 0.3])
        ...
        % else
        %disp('Error: Old Points not in Current Set')
        % end
...
xlabel('m')
ylabel('n')
title('3h Integrals for XY Plane')
ylabel(cb,'Values')
hold on
plot([N N],[-0.5 N],'k')
plot([-0.5 N],[N N],'k')
